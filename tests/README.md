...
`make test` builds the CLI, runs `inspect`, `audit`, tokenizer piece/encode/decode, `format-chat`, tensor table/mmap inspection, Qwen3 binding, tiny text-only generation, vision raw tensor smoke, sampler/chat CLI, and scalar and quant kernel golden smoke tests; it also confirms truncated fixtures fail cleanly and checks unsupported-schema diagnostics without reading tensor data.
...
Vision smoke coverage uses the tiny raw image fixture to validate patch embedding, a one-block vision transformer, resampler/projector binding, image embedding span injection, stable checksum `0xbab3d2ad`, placeholder/span alignment, and observable image-vs-text logits difference.

...
- [x] 支持 Qwen3/MiniCPM-o 4.5 的 rope、q/k norm、GQA、KV cache layout 和 chat template。
  - [x] 当前 forward 已包含 q/k norm、GQA 维度、KV cache 和预计算 RoPE 的基本执行路径，能跑官方模型 smoke。
  - [x] 与 llama.cpp 对齐 RoPE scaling/chat template/stop token 细节。
...
- [x] 移除 tiny audio cap，支持真实用户语音长度和多 chunk 输入。
  ...
  - [x] `audio2audio-real --test-prefix PREFIX --count N --out ...` 真实路径现已支持多 user turn 会话；audio encoder GGUF/filterbank、reference embedding 与 Qwen runtime/KV cache 会在整场会话内复用，不再按 turn 重载。
...
- [x] 实现正确 prompt 协议：reference audio 进入 system prompt，user audio 进入 `<|im_start|>user` turn。
  - [x] `audio2audio-real --audit-only` 已区分 `ref_audio` 和 `user_audio`，并在协议日志中输出两者路径。
  - [x] `audio2audio-real --text-only/--no-tts` 已按 `llama.cpp-omni` 的非双工顺序拼装 system ref-audio 前缀、user audio turn 和 assistant prompt。
- [x] 支持 `prefix_0000.wav`/`prefix_0001.wav` 测试格式，也支持显式 `--ref-audio`/`--user-audio` 参数。
- [x] 保存 audio embedding checksum、span count、n_pos、prefill timing 便于与 llama.cpp-omni 对照。
  ...
  - [x] `audio2audio-real --text-only/--no-tts` 现已输出 `ref/user n_pos`、embedding checksum、load/prefill/decode timing。
...
- [x] 解析并绑定 `token2wav-gguf/encoder.gguf`。
- [x] 解析并绑定 `token2wav-gguf/flow_matching.gguf`。
- [x] 解析并绑定 `token2wav-gguf/flow_extra.gguf`。
...
- [x] 支持 llama.cpp-omni 测试格式：`--test-prefix PREFIX --count N`，其中 `0000` 是 ref，`0001..` 是 user turn。
  ...
  - [x] `--test-prefix PREFIX --count N` 现已覆盖 audit、`--text-only` 与完整 `--out` 路径。
  - [x] `--text-only/--no-tts --test-prefix PREFIX --count N` 会逐个读取 `PREFIX_0001.wav..PREFIX_%04u.wav`，把它们当作同一 user query 的连续音频 chunk。
  - [x] 完整 `--out` 路径会把多 turn 输出按 `turn_0001_*`、`turn_0002_*` 前缀分桶，最后一轮仍可写回显式 `--out` alias。
...
- [x] 文档记录当前速度预期：第一版先正确，再优化，不承诺立即达到 llama.cpp-omni 性能。
  - [x] `docs/audio2audio-real.md` 已明确当前是 correctness-first 的 CPU reference `tts2wav`，并保留 surrogate `mu` / 非精确 HiFiGAN2 parity 的限制说明。

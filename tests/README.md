...
- `tts-token-align` is manual and wraps the simplex probe with per-chunk `count`, `prefix_match`, and `exact` summaries against a reference `tts_wav` directory. It now also accepts a local seeded-baseline manifest entry and can run strict prefix-oracle comparisons with `--allow-max-audio-tokens-stop`.
...
- `audio2audio-real-baseline-compare` is manual and compares one generated Uya report JSON against a named entry in the local `outputs/baselines/audio2audio_baselines.json` manifest. It can also write a persisted compare JSON for local CPU-only baseline tracking.
...
Real TTS decoder coverage is also manual. The local seeded-baseline manifest format is `outputs/baselines/tts_token_baselines.json`, and `make tts-seeded-baseline-record` plus `make tts-token-align` now support fixed-seed / greedy strict-oracle runs without relying on the old unpinned `round_000/tts_wav` dumps.
...
```text
bench config: mode=text-real backend=cpu prompt_tokens=4 gen_tokens=4 repetitions=1 warmup=0 threads=1
bench load: ms=...

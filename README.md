...
- Qwen3 text `generate` / `chat` / real-text `bench` now accept `--backend cpu|cuda`. The current CUDA pass is real, Linux/NVIDIA-only, fixed to `device.index=0`, and offloads Qwen3 text matvecs through cuBLAS while attention and KV cache stay on the host.
...
The current CUDA backend is intentionally narrow and explicit:

- Scope: `Qwen3` pure-text path only.
- Covered on CUDA: dense `F32` matvecs directly; dense `F16` / `BF16` via dense upload; `Q4_K` / `Q6_K` via native `Q8_1 + vecdot` matvec when NVRTC kernels are available, with explicit fallback to host-side `F16` staging when they are not.
- Also covered on CUDA for tokenwise decode: causal single-token attention with device-resident KV cache.
- Still on CPU: token embedding lookup, sampler, and all vision/audio/speech/token2wav paths.
- Platform: Linux + NVIDIA CUDA, single GPU only, `device.index=0`.
- Current quant policy: prefer `native_q8_1_vecdot` with a `persistent_prefix_lru_resident` device cache policy, and keep `host_f16_staging` as the explicit fallback path when native quant kernels are unavailable. This removes most activation-side H2D overhead on real `Q4_K/Q6_K` probes while keeping the implementation honest about what still runs on CPU.
- Fallback policy: `--backend cuda` never silently falls back to CPU. Unsupported dtype/library/device errors are reported and the command exits.

Useful commands:

```sh
build/minicpm-o-uya cuda-smoke
build/minicpm-o-uya generate /path/to/qwen3.gguf "hello" --max-new-tokens 32 --backend cpu
build/minicpm-o-uya generate /path/to/qwen3.gguf "hello" --max-new-tokens 32 --backend cuda
build/minicpm-o-uya bench /path/to/qwen3.gguf --n-prompt 512 --n-gen 128 --repetitions 5 --threads 28 --backend cuda
```
...
| `generate <model.gguf> <prompt> [sampler args...]` | Tiny text generation smoke | Deterministic reference path; supports `--backend cpu|cuda` for Qwen3 text |
...
| `bench <model.gguf> [--prompt TEXT] [--n-prompt N] [--n-gen N] [--repetitions N] [--threads N] [--no-warmup] [--backend cpu|cuda]` | Real text benchmark | Measures load/prefill/decode for Qwen3 path with live timings and backend/transfer counters |
...
```text
bench config: mode=text-real backend=cpu|cuda ...
bench load: ms=...
bench text_prompt: ... tokens/s ...
bench text_decode: ... tokens/s ...
bench first_token: us=...
bench backend: hot_matvec=...
bench transfer: backend_init_ms=... host_to_device_bytes=... device_to_host_bytes=...
bench memory: peak_estimate_bytes=...
```

On the local `MiniCPM-o-4_5-Q4_K_M.gguf` path, the current real comparison is possible and decode already benefits. With native `Q8_1 + vecdot` enabled on this host, the `1 prompt + 1 decode` probe currently looks like:

```text
cpu:  text_decode ~= 0.102 tok/s
cuda: text_decode ~= 1.749 tok/s
```

The same CUDA run currently reports `quant_path=native_q8_1_vecdot`, `attention=cuda`, `kv_cache=device`, `device_cache=persistent_prefix_lru_resident`, and `host_to_device_bytes~=4.67 GB` for that `1 prompt + 1 decode` probe. Single-token decode now keeps RoPE/RMSNorm and the intermediate activation flow on device, so the next major bottlenecks are the remaining weight-upload traffic plus sampler/embedding CPU work, not the old `Q4_K/Q6_K` host staging path.
...
| `backend=cuda ... supported_dtypes=F32|F16|BF16` | CUDA text path hit an unsupported Qwen3 matvec tensor dtype | Use `--backend cpu` for that model or extend CUDA coverage |
| `unable to load gpu helper` / `unable to load libcuda.so` | Missing helper `.so`, NVIDIA driver, or CUDA runtime visibility | Rebuild, check `build/libminicpmo_gpu_helper.so`, and confirm `nvidia-smi` works |

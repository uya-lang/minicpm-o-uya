...
The same real TTS decoder/audio-token path is now reachable from `audio2audio-real` itself. In full mode the command chunks its generated answer tokens, reuses the TTS KV cache across chunks, and writes `audio_tokens_chunk_*.txt/bin` beside the requested `--out` path before the later token2wav/HiFiGAN stage.
